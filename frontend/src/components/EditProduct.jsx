...
import { uploadImage } from "../helper/baseURL";import { baseURL } from "../helper/baseURL";
export default function EditProduct({updateProduct, onClose, callFunction}) {
 ...
 const handleSubmitData=async(ev)=>{
  ...
  return (
    <div className=" bg-gray-900  bg-opacity-70 w-full h-full absolute inset-0  "  onClick={onClose}>
      <div className="flex justify-between w-[80%] mx-auto  top-[-4px] pt-4 px-3 
       items-start absolute mt-2 inset-x-3 in bg-white box-border" onClick={ev=>ev.stopPropagation()}>
          ...
          <button
            className="bg-red-500 text-white rounded-full p-1 text-2xl"
            onClick={onClose}
          >
            <IoIosClose />
          </button>
      ...
      <div className="max-w-[80%] h-full mx-auto  bg-white overflow-scroll" onClick={ev=>ev.stopPropagation()}>
        ...
        <form onSubmit={(ev)=>{onClose(),handleSubmitData(ev)}} className="w-[70%] mx-auto mt-16 px-2 pb-10 h-full " 
        >
          ...
          <button className="bg-sky-500 text-white rounded-lg p-2 hover:bg-sky-600" >Edit Product</button>
        </form> 
